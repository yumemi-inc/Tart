...
実装上の足場はすでに揃っている。
[StoreImpl.kt:131](../../tart-core/src/commonMain/kotlin/io/yumemi/tart/core/StoreImpl.kt) の `stateRuntimes` が state クラスごとに `StateRuntime` を持ち、
[StoreImpl.kt:143](../../tart-core/src/commonMain/kotlin/io/yumemi/tart/core/StoreImpl.kt) の `StateRuntime` は `scope` と `actionLaunchJobs` を保持する。
enter / action の `launch {}` は [StoreImpl.kt:443](../../tart-core/src/commonMain/kotlin/io/yumemi/tart/core/StoreImpl.kt) の `launchInStateRuntime()` を経由してこの scope 上で起動される。
...
TestDispatcher を使わない方針を取る以上、この区別は Tart 側で何らかの形で持ち込む必要がある。
...
- `tart-core` の [`StoreInternalApi`](../../tart-core/src/commonMain/kotlin/io/yumemi/tart/core/StoreInternalApi.kt) (`@InternalTartApi`) に `suspend fun awaitIdle()` を追加する
- `StoreImpl` で実装する
- `tart-test` の [`StoreExtensions`](../../tart-test/src/commonMain/kotlin/io/yumemi/tart/test/StoreExtensions.kt) に public extension を置き、`requireStoreInternalApi().awaitIdle()` に委譲する
...
- `tart-test` の extension 名。`awaitIdle()` のまま出すか、`startAndWait()` / `dispatchAndWait()` の語感に合わせた別名にするか
